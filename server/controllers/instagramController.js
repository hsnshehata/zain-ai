...
const sendMessage = (recipientId, messageText, accessToken, imageUrl = null) => {
  return new Promise((resolve, reject) => {
    ...
    request({
      ...
      console.log(`[${getTimestamp()}] ✅ Message sent to ${recipientId}: ${messageText}${imageUrl ? ` with image ${imageUrl}` : ''}`);
      resolve(body);
    });
  });
};
      ...
      if (entry.messaging) {
        for (const event of entry.messaging) {
          ...
          if (event.reaction && bot.instagramMessageReactionsEnabled) {
            console.log(`[${getTimestamp()}] 📩 Processing reaction event from ${prefixedSenderId}: ${event.reaction.reaction}`);
          ...
          if (event.referral && bot.instagramMessagingReferralsEnabled) {
            console.log(`[${getTimestamp()}] 📩 Processing referral event from ${prefixedSenderId}: ${event.referral.ref}`);
