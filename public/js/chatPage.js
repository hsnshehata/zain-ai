...
<iframe src="${data.link}" style="width: 100%; height: 100%; border: none;" scrolling="no"></iframe>
          ...
          // Make the preview controls non-interactive to avoid accidental editing from the preview area.
          try {
            if (previewMessageInput) previewMessageInput.setAttribute('readonly', 'readonly');
            if (previewImageInput) previewImageInput.setAttribute('disabled', 'disabled');
            if (previewImageInputBtn) previewImageInputBtn.style.pointerEvents = 'none';
            if (previewSendMessageBtn) previewSendMessageBtn.disabled = true;
          } catch (e) {
            console.warn('Could not disable preview interactive controls', e);
          }
