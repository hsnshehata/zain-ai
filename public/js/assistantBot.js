...
رابط صفحة الدردشة الخاص بالمستخدم  src="${data.link}" 
### تعليمات عامة:
...
`;
  ...
  function parseSimpleCommand(message) {
    const navigateMatch = message.match(/(?:انتقل إلى|انتقل الي|افتح|افتح صفحة|ممكن تفتح) صفحة (القواعد|الرسايل|الرسائل|التقييمات|إعدادات فيسبوك|الفيسبوك|البوتات|التحليلات|تخصيص الدردشة)/i);
    if (navigateMatch) {
      const pageMap = {
        'القواعد': 'rules',
        'الرسايل': 'messages',
