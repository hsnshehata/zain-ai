...
document.addEventListener("DOMContentLoaded", () => {
  async function loadInstagramPage() {
          ...
          <ul>
            <li>
              ...
              <span style="display: block; margin-top: 5px;">
                ...
                3. سجّل دخولك بنفس حساب فيسبوك اللي مرتبط بحساب الإنستجرام بتاعك.<br>
        ...
        <div class="card settings-card">
          ...
          <div class="card-body toggles-grid">
            <div class="setting-item toggle-item">
              ...
              <label class="switch">
                <input type="checkbox" id="messagingOptinsToggle" data-setting-key="instagramMessagingOptinsEnabled">
            ...
            <div class="setting-item toggle-item">
              ...
              <label class="switch">
                <input type="checkbox" id="messageReactionsToggle" data-setting-key="instagramMessageReactionsEnabled">
                <span class="slider"></span>
              </label>
            </div>
            <div class="setting-item toggle-item">
              ...
              <label class="switch">
                <input type="checkbox" id="messagingReferralsToggle" data-setting-key="instagramMessagingReferralsEnabled">
                <span class="slider"></span>
              </label>
            </div>
            <div class="setting-item toggle-item">
              ...
              <label class="switch">
                <input type="checkbox" id="messageEditsToggle" data-setting-key="instagramMessageEditsEnabled">
                <span class="slider"></span>
              </label>
            </div>
            <div class="setting-item toggle-item">
              <div class="setting-info">
                ...
                <p>تسمح للبوت بوضع تصنيفات وتعديل حالات المحادثة (يتطلب إعدادات صلاحيات خاصة).</p>
              </div>
              <label class="switch">
                <input type="checkbox" id="inboxLabelsToggle" data-setting-key="instagramInboxLabelsEnabled">
                <span class="slider"></span>
              </label>
            </div>
            <div class="setting-item toggle-item">
              ...
              <label class="switch">
                <input type="checkbox" id="commentsRepliesToggle" data-setting-key="instagramCommentsRepliesEnabled">
                <span class="slider"></span>
              </label>
            </div>
          </div>
    ...
    // Load Facebook SDK (used for Instagram as well)
    ...
    // Initialize Facebook SDK (used for Instagram)
    ...
    function getUserInstagramAccounts(accessToken) {
      FB.api('/me/accounts', { access_token: accessToken }, function (response) {
        if (response && !response.error) {
          const facebookPages = response.data;
          if (facebookPages.length === 0) {
            errorMessage.textContent = 'لم يتم العثور على صفحات فيسبوك مرتبطة بحسابك';
            errorMessage.style.display = 'block';
            return;
          }
          ...
          facebookPages.forEach(page => {
            FB.api(`/${page.id}/instagram_accounts`, { access_token: page.access_token }, function (igResponse) {
              processed++;
              if (igResponse && !igResponse.error && igResponse.data.length > 0) {
              ...
              if (processed === facebookPages.length) {
                ...
                showInstagramAccountModal(instagramAccounts);
              }
            });
          });
        } else {
    ...
    function showInstagramAccountModal(accounts) {
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <select id="accountSelect" class="form-control">
              ...
              ${accounts.map(account => `<option value="${account.id}" data-token="${account.access_token}">${account.username}</option>`).join('')}
            </select>
          </div>
