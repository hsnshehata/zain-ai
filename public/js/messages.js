...
document.addEventListener("DOMContentLoaded", () => {
  async function loadMessagesPage() {
    ...
    let currentPage = 1;
    const itemsPerPage = 20; // Number of conversation cards per page
    ...
    // Add event listeners for edit/save buttons within the modal
  ...
  window.loadMessagesPage = loadMessagesPage;
    ...
    function handleEditRuleClick(event) {
        ...
        // Hide the edit button's container and show the edit area
        button.closest(".message-actions").style.display = "none";
        editArea.style.display = "block";
        textarea.value = answer; // Pre-fill textarea
        textarea.focus();
    }
    ...
    function handleCancelEditClick(event) {
        ...
        const messageActions = editArea.previousElementSibling; // Assuming actions div is right before edit area
    ...
    async function handleSaveEditedRuleClick(event) {
        ...
        try {
            ...
            // Hide edit area and show original button container
            editArea.style.display = "none";
            if (messageActions) messageActions.style.display = "block";

        } catch (err) {
            console.error("Error saving new rule:", err);
            alert(`فشل حفظ القاعدة: ${err.message}`);
            if (err.message === "جلسة غير صالحة") logoutUser();
        } finally {
            // Reset button states regardless of success/failure
    ...
    // Remove or comment out the old handleSaveRuleClick function if it exists
    /* async function handleSaveRuleClick(event) { ... } */
