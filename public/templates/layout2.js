(function(){
  ...
  function renderFeatured(products, enableCart){
    const top = products.slice(0,12).map(p=>productCard(p, enableCart)).join('');
    return `
      <section id="featured" class="section featured-mosaic">
        <div class="featured-head">
          <h2 class="section-title" style="margin:0;">منتجاتنا المميزة</h2>
        </div>
        <div class="featured-frame">
          <div class="featured-grid">${top}</div>
        </div>
      </section>`;
  }
  ...
  function attachInteractions(root, ctx){
    ...
    const bannerImg = root.querySelector('.banner-hero .banner-img');
    const onScroll = () => {
      if (header){ if (window.scrollY>10) header.classList.add('scrolled'); else header.classList.remove('scrolled'); }
      if (bannerImg){ const y = window.scrollY||0; const t = Math.min(60, y*0.20); bannerImg.style.transform = `translateY(${t}px) scale(1.08)`; }
    };
    ...
    // أسهم سحب لمنتجاتنا المميزة
    const fTrack = root.querySelector('#featured .featured-track');
    const fPrev = root.querySelector('#featured .featured-prev');
    const fNext = root.querySelector('#featured .featured-next');
    if (fTrack && fPrev && fNext){
      const by = () => Math.max(260, Math.floor(fTrack.clientWidth * 0.9));
      fPrev.addEventListener('click', ()=> fTrack.scrollBy({ left: -by(), behavior: 'smooth' }));
      fNext.addEventListener('click', ()=> fTrack.scrollBy({ left: by(), behavior: 'smooth' }));
    }
    ...
    if (catBar){
      catBar.addEventListener('click', (e)=>{
        ...
        if (!pill) return;
        if (!pill.hasAttribute('data-cid')) return; // تجاهل الروابط الإضافية
  ...
  function applyFilters(root, ctx){
    ...
    // فلترة بالقسم المختار
    const catSelected = UI_STATE.currentCategory && UI_STATE.currentCategory !== 'all';
    if (catSelected){
    ...
    if (UI_STATE.query) title.textContent = 'نتائج البحث';
      ...
      title.textContent = cat?.name || 'كل المنتجات';
    ...
    // إخفاء قسم المميزة كذلك في حالة اختيار قسم
    const feat = root.querySelector('#featured');
    if (feat){ feat.classList.toggle('hidden', !!searchMode || !!catSelected); }
  }
